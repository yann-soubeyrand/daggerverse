...
type testOptionImpl struct {
	query *querybuilder.Selection
	id    *TestOptionID
}

type TestOptionID string

func LoadTestOptionFromID(r *Client, id TestOptionID) TestOption {
	q := querybuilder.Query().Client(r.GraphQLClient())
	q = q.Select("loadTestTestOptionFromID")
	q = q.Arg("id", id)
	return &testOptionImpl{query: q}
}

func (r *testOptionImpl) WithGraphQLQuery(q *querybuilder.Selection) TestOption {
	return &testOptionImpl{query: q}
}

func (r *testOptionImpl) XXX_GraphQLType() string {
	return "TestOption"
}

func (r *testOptionImpl) XXX_GraphQLIDType() string {
	return "TestOptionID"
}

func (r *testOptionImpl) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *testOptionImpl) MarshalJSON() ([]byte, error) {
	if r == nil {
		return []byte("\"\""), nil
	}
...
func (r *testOptionImpl) UnmarshalJSON(bs []byte) error {
	var id TestOptionID
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *LoadTestOptionFromID(dag, id).(*testOptionImpl)
	return nil
}

func (r *testOptionImpl) toIface() TestOption {
	if r == nil {
		return nil
	}
	return r
...
func (r *testOptionImpl) ID(ctx context.Context) (TestOptionID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")
	var response TestOptionID
	q = q.Bind(&response)
	return response, q.Execute(ctx)
}

func (r *testOptionImpl) Do(m *Test) *Test {
	q := r.query.Select("do")
	q = q.Arg("m", m)

	return (&Test{}).WithGraphQLQuery(q)
}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Test":
		...
		case "Container":
			...
			return (*Test).Container(&parent), nil
		case "":
			var parent Test
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var opts []*testOptionImpl
			if inputArgs["opts"] != nil {
				err = json.Unmarshal([]byte(inputArgs["opts"]), &opts)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg opts", err))
				}
			}
			return New(convertSlice(opts, (*testOptionImpl).toIface)...), nil
		default:
			...
			WithObject(
				dag.TypeDef().WithObject("Test").
					...
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Test")).
							WithArg("opts", dag.TypeDef().WithListOf(dag.TypeDef().WithInterface("TestOption"))))).
			WithInterface(
				dag.TypeDef().WithInterface("TestOption").
					WithFunction(
						dag.Function("Do",
							dag.TypeDef().WithObject("Test")).
							WithArg("m", dag.TypeDef().WithObject("Test")))), nil
	default:
